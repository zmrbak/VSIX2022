...
namespace VSIX31
{
    ...
    internal sealed class ToolWindow1Command
    {
        ...
        /// </summary>
        public static readonly Guid CommandSet = new Guid("5022a30d-7c75-4d1c-ba2f-ad33ba87f5aa");
            ...
            _ = this.package.JoinableTaskFactory.RunAsync(async delegate
              {
                  for (int i = 0; i < 10; i++)
                  {
                      ToolWindowPane window = await this.package.ShowToolWindowAsync(typeof(ToolWindow1), i, false, this.package.DisposalToken);

                      if(window==null)
                      {
                          window = await this.package.ShowToolWindowAsync(typeof(ToolWindow1), i, true, this.package.DisposalToken);
                          if ((null == window) || (null == window.Frame))
                          {
                              throw new NotSupportedException("Cannot create tool window");
                          }

                          window.Caption = "Window: " + i;
                          break;
                      }                      
                  }                  
              });
